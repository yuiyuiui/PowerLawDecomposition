@testset "wynn_epsilon_core" begin
    @testset "correctness — geometric series (3 terms)" begin
        # With 3 partial sums the algorithm hits the exact limit at column 2
        r = 0.5
        S = cumsum([r^k for k in 0:2])  # [1, 1.5, 1.75]
        result = wynn_epsilon_core(S)
        @test isapprox(result, 1 / (1 - r); atol=1e-12)
    end

    @testset "correctness — alternating series e⁻¹" begin
        # sum (-1)^k/k! converges to e⁻¹
        terms = [(-1.0)^k / factorial(k) for k in 0:12]
        S = cumsum(terms)  # 13 elements (odd)
        result = wynn_epsilon_core(S)
        @test isapprox(result, exp(-1.0); atol=1e-10)
    end

    @testset "correctness — alternating series ln(2)" begin
        # sum (-1)^(k+1)/k = ln(2)
        terms = [(-1)^(k + 1) / k for k in 1:15]
        S = cumsum(terms)  # 15 elements (odd)
        result = wynn_epsilon_core(S)
        @test isapprox(result, log(2); atol=1e-10)
    end

    @testset "correctness — pi/4 Leibniz series" begin
        terms = [(-1)^k / (2k + 1) for k in 0:20]
        S = cumsum(terms)  # 21 elements (odd)
        result = wynn_epsilon_core(S)
        @test isapprox(result, π / 4; atol=1e-10)
    end

    @testset "type stability" begin
        S64 = cumsum([0.5^k for k in 0:6])  # 7 elements (odd)
        @test @inferred(wynn_epsilon_core(S64)) isa Float64

        S32 = Float32.(S64)
        @test @inferred(wynn_epsilon_core(S32)) isa Float32
    end

    @testset "boundary — length 3" begin
        S = cumsum([0.5^k for k in 0:2])
        result = wynn_epsilon_core(S)
        @test isfinite(result)
    end

    @testset "boundary — length 1" begin
        S = [3.14]
        result = wynn_epsilon_core(S)
        @test result == 3.14
    end

    @testset "edge — already converged" begin
        S = fill(2.0, 7)
        result = wynn_epsilon_core(S)
        @test isfinite(result)
    end

    @testset "precision — Float32 vs Float64" begin
        terms64 = [(-1)^(k + 1) / k for k in 1:11]
        S64 = cumsum(terms64)  # 11 elements (odd)
        S32 = Float32.(S64)
        exact = log(2)

        res64 = wynn_epsilon_core(S64)
        res32 = wynn_epsilon_core(S32)
        @test abs(res64 - exact) < 1e-8
        @test abs(res32 - exact) < 1e-4
    end
end
